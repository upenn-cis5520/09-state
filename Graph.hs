...
reflexive g = all getAll $ foldGraph f g where
  f n = All (n `elem` adjacent g n)

-- is symmetric?
symmetric :: Graph -> Bool
symmetric g = all getAll $ foldGraph f g where
  f n = All (all (\v -> n `elem` adjacent g v) (adjacent g n))
